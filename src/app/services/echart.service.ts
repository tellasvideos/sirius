...
export class EchartService {
  ...
  get_Inquerito_EchartData(): Observable<BasicEchartLineModel[]> {
    ...
    return this.httpClient.get<BasicEchartLineModel[]>('http://localhost:8001/api/v1/getinquirerformsnumberpermonth/', { headers: headers });
  }
  ...
  get_Interesses_EchartData(): Observable<BasicEchartLineModel[]> {
    ...
    headers = headers.append('Content-Type', 'application/json');
    headers = headers.append('Authorization', 'Token ' + String(localStorage.getItem('userToken')));
    return this.httpClient.get<BasicEchartLineModel[]>('http://localhost:8001/api/v1/getinterestexpressionsnumberpermonth/', { headers: headers });
  }
  ...
  get_CadeiaValor_EchartData(): Observable<BasicEchartLineModel[]> {
    ...
    headers = headers.append('Content-Type', 'application/json');
    headers = headers.append('Authorization', 'Token ' + String(localStorage.getItem('userToken')));
    return this.httpClient.get<BasicEchartLineModel[]>('http://localhost:8001/api/v1/getvaluechainfrominterestexpressions/', { headers: headers });
  }
